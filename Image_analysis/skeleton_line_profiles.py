...
class SkeletonCurvature:
    def __init__(self, im_path, nellie_path, save_path=None, smoothing_factors=None, manual_smoothing=False, timepoints=None,
        ...
        self.best_intensity_resolution_factor = None
        self.best_structural_resolution_factor = None
    ...
    def calculate_local_curvature(self, cumulative_distances, values, spline_resolution=25, smoothing_factor=0.0):
        ...
        dx_du, dy_du = splev(u_new, tck, der=1)  # First derivative
        d2x_du2, d2y_du2 = splev(u_new, tck, der=2)  # Second derivative

        # Calculate curvature using the curvature formula for 2D parametric curves
        curvature = np.abs(dx_du * d2y_du2 - dy_du * d2x_du2) / (dx_du ** 2 + dy_du ** 2) ** (3 / 2)

        # Ensure we do not divide by zero in curvature calculation
    ...
    def find_optimal_smoothing_and_resolution(self, skeleton_im, intensity_or_structural_im, data_type='intensity'):
        """
        Finds the optimal smoothing factor by maximizing the difference in weighted mean curvature
        between the peak and average timepoints, ensuring that the peak timepoint has a higher curvature.
        Keeps the spline resolution factor fixed at 1, but allows interactive adjustment later.
        ...
        Returns:
            tuple: The optimal smoothing factor and spline resolution factor.
        """
        ...
        self.pearl_timepoint = int(self.display_frames[1])
        ...
        # Define range for smoothing factors
        smoothing_factors = np.linspace(1, 0, 25)  # Start from 1 and work down
        resolution_factor = 1.0  # Fixed resolution factor
        ...
        # Iterate over smoothing factors only
        for sf in smoothing_factors:
            ...
            for label in unique_labels:
                ...
                if self.im_info.no_z:
                    ...
                    scaling = (
                    peak_timepoint, self.im_info.dim_res['Z'], self.im_info.dim_res['Y'], self.im_info.dim_res['X'])

                cumulative_distances_peak = self.getCumulativeDistances(skeleton_path, scaling=scaling)
                if cumulative_distances_peak is None:
                    continue

                # Keep the resolution factor fixed at 1.0
                spline_resolution_peak = int(len(label_coords) * resolution_factor)
                curvature_peak, spline_x_peak, spline_y_peak, mean_curvature_peak = self.calculate_local_curvature(
                    cumulative_distances_peak, data_vals_peak, spline_resolution=spline_resolution_peak,
                    smoothing_factor=sf)
            ...
            for label in unique_labels:
                ...
                if self.im_info.no_z:
                    ...
                    scaling = (
                    avg_timepoint, self.im_info.dim_res['Z'], self.im_info.dim_res['Y'], self.im_info.dim_res['X'])
                ...
                # Keep the resolution factor fixed at 1.0
                spline_resolution_avg = int(len(label_coords) * resolution_factor)
                curvature_avg, spline_x_avg, spline_y_avg, mean_curvature_avg = self.calculate_local_curvature(
                    cumulative_distances_avg, data_vals_avg, spline_resolution=spline_resolution_avg,
                    smoothing_factor=sf)
            ...
            if weighted_mean_curvature_peak > weighted_mean_curvature_avg:
                ...
                # If this score is better, update the best score, smoothing factor
                if score > best_score:
                    best_score = score
        ...
        print(f"Best smoothing factor: {best_smoothing_factor}")
        print(f"Fixed resolution factor: {resolution_factor}")

        # Plot and interactively adjust the smoothing factor and resolution
        if self.manual_smoothing:
            ...
            if longest_traversal_peak is None:
                longest_traversal_peak = label_traversals_peak[0]
            if longest_traversal_avg is None:
                longest_traversal_avg = label_traversals_avg[0]
            ...
            title = f"Adjust Smoothing Factor and Resolution for {data_type} Data"
            ...
            best_smoothing_factor, best_resolution_factor = self.interactive_smoothing_plot(
                longest_traversal_peak, longest_traversal_avg, best_smoothing_factor, resolution_factor, title)
        else:
            best_smoothing_factor =1.0
            best_resolution_factor = 1.0

        return best_smoothing_factor, best_resolution_factor  # Return the best smoothing and resolution factors

    # def find_optimal_smoothing_and_resolution(self, skeleton_im, intensity_or_structural_im, data_type='intensity'):
    #     """
    #     Finds the optimal smoothing factor and spline resolution by maximizing the difference in weighted mean curvature
    #     between the peak and average timepoints, ensuring that the peak timepoint has a higher curvature.
    #
    #     Args:
    #         skeleton_im (ndarray): The skeleton image array.
    ...
    #     frame_data_im_avg = intensity_or_structural_im[avg_timepoint:avg_timepoint + 1]
    #
    #     # Define ranges for smoothing factors and resolution factors
    #     smoothing_factors = np.linspace(1, 0, 25)  # Start from 1 and work down
    #     resolution_factors = np.linspace(0.2, 2, 10)  # Factors to adjust spline resolution
    #
    #     best_smoothing_factor = 1.0
    #     best_resolution_factor = 1.0
    #     best_score = -np.inf
    ...
    #     longest_traversal_avg = None
    #
    #     # Iterate over smoothing factors and resolution factors
    #     for sf in smoothing_factors:
    ...
    #                     continue
    #
    #                 spline_resolution_peak = int(len(label_coords) * rf)  # Adjusted resolution
    #                 curvature_peak, spline_x_peak, spline_y_peak, mean_curvature_peak = self.calculate_local_curvature(
    #                     cumulative_distances_peak, data_vals_peak, spline_resolution=spline_resolution_peak,
    #                     smoothing_factor=sf)
    #
    ...
    #                     continue
    #
    #                 spline_resolution_avg = int(len(label_coords) * rf)  # Adjusted resolution
    #                 curvature_avg, spline_x_avg, spline_y_avg, mean_curvature_avg = self.calculate_local_curvature(
    #                     cumulative_distances_avg, data_vals_avg, spline_resolution=spline_resolution_avg,
    #                     smoothing_factor=sf)
    #
    ...
    #
    #             # Calculate weighted mean curvatures for both timepoints
    #             weighted_mean_curvature_peak = self.compute_weighted_mean_curvatures(label_traversals_peak)[
    #                 peak_timepoint]
    #             weighted_mean_curvature_avg = self.compute_weighted_mean_curvatures(label_traversals_avg)[avg_timepoint]
    #
    #             # Calculate the score as the difference between weighted mean curvatures, ensuring peak > avg
    ...
    #                 score = weighted_mean_curvature_peak - weighted_mean_curvature_avg
    #
    #                 # If this score is better, update the best score, smoothing factor, and resolution factor
    #                 if score > best_score:
    #                     best_score = score
    #                     best_smoothing_factor = sf
    #                     best_resolution_factor = rf
    #                     best_traversals_peak = label_traversals_peak
    #                     best_traversals_avg = label_traversals_avg
    #
    #                     # Find the longest traversal for plotting
    ...
    #             f"No suitable labels found in the peak or average timepoint to determine the smoothing factor for {data_type} data.")
    #     print(f"Best smoothing factor: {best_smoothing_factor}")
    #     print(f"Best resolution factor: {best_resolution_factor}")
    #
    #     # Plot and interactively adjust the smoothing factor and resolution
    #     if self.manual_smoothing:
    #         # Fallback to use the first traversal if the longest traversal is None
    ...
    #         # Interactive adjustment
    #         best_smoothing_factor, best_resolution_factor = self.interactive_smoothing_plot(
    #             longest_traversal_peak, longest_traversal_avg, best_smoothing_factor, best_resolution_factor, title)
    #
    #     return best_smoothing_factor, best_resolution_factor  # Return the best smoothing and resolution factors

    def interactive_smoothing_plot(self, traversals_peak, traversals_avg, best_smoothing_factor, best_resolution_factor, title):
        import matplotlib.pyplot as plt
        from matplotlib.widgets import Slider

        """
        Creates an interactive plot with sliders to adjust the smoothing factor and spline resolution, 
        allowing the user to see the effect on the fitted splines.
        ...
        peak_line, = ax.plot(traversals_peak.spline_x, traversals_peak.spline_y, color='blue',
                             linestyle='-', label='Peak Spline')
        avg_line, = ax.plot(traversals_avg.spline_x, traversals_avg.spline_y, color='red',
        ...
        ax_smooth = plt.axes([0.1, 0.15, 0.8, 0.03])
        smooth_slider = Slider(ax_smooth, 'Smoothing Factor', 0, 1, valinit=best_smoothing_factor, valstep=0.01)

        # Slider for spline resolution factor
        ax_resolution = plt.axes([0.1, 0.1, 0.8, 0.03])
        resolution_slider = Slider(ax_resolution, 'Resolution Factor', 0.1, 5, valinit=best_resolution_factor, valstep=0.1)

        # Initialize the final smoothing factor and resolution factor
        final_smoothing_factor = [best_smoothing_factor]
        final_resolution_factor = [best_resolution_factor]

        def update(val):
            sf = smooth_slider.val
            rf = resolution_slider.val
            # Update the fitted splines for the peak and average timepoints with the new smoothing factor and resolution factor
            new_spline_resolution_peak = int(len(traversals_peak.cumulative_distances) * rf)
            new_curvature_peak, spline_x_peak, spline_y_peak, _ = self.calculate_local_curvature(
                traversals_peak.cumulative_distances, traversals_peak.intensity_values,
                spline_resolution=new_spline_resolution_peak, smoothing_factor=sf)

            new_spline_resolution_avg = int(len(traversals_avg.cumulative_distances) * rf)
            new_curvature_avg, spline_x_avg, spline_y_avg, _ = self.calculate_local_curvature(
                traversals_avg.cumulative_distances, traversals_avg.intensity_values,
                spline_resolution=new_spline_resolution_avg, smoothing_factor=sf)

            # Update plot data
            peak_line.set_data(spline_x_peak, spline_y_peak)
            avg_line.set_data(spline_x_avg, spline_y_avg)
            fig.canvas.draw_idle()

        # Register update function with both sliders
        ...
        def on_close(event):
            nonlocal final_smoothing_factor, final_resolution_factor
            plt.close(fig)  # Close the figure
            final_smoothing_factor[0] = smooth_slider.val  # Capture the final value from the slider
            final_resolution_factor[0] = resolution_slider.val  # Capture the final value from the slider
        ...
        return final_smoothing_factor[0], final_resolution_factor[0]  # Return the final smoothing factor and resolution factor
    ...
    def plot_mean_curvature_over_time(self, weighted_mean_curvatures, title='Weighted Mean Curvature Over Time',
        ...
        plt.grid(True)
        plt.legend()
        plt.axvline(int(self.display_frames[1]), color="darkorange", linestyle='--', linewidth=2)  # Line at the pearl point
    ...
    def run_curvature(self):
        ...
        if self.smoothing_factor_intensity is None:
            ...
            self.smoothing_factor_intensity, self.best_intensity_resolution_factor = self.find_optimal_smoothing_and_resolution(skeleton_im,intensity_im,
                                                                                               data_type='intensity')
            ...
            print(f"Found optimal intensity smoothing factor: {self.smoothing_factor_intensity} and resolution factor: {self.best_intensity_resolution_factor}")
        else:
            self.best_intensity_resolution_factor = 1.0
        ...
        if self.smoothing_factor_structural is None:
            ...
            self.smoothing_factor_structural, self.best_structural_resolution_factor = self.find_optimal_smoothing_and_resolution(skeleton_im,structural_im,
            ...
            print(f"Found optimal structural smoothing factor: {self.smoothing_factor_structural} and resolution factor: {self.best_structural_resolution_factor}")
        else:
            self.best_structural_resolution_factor = 1.0
        ...
        for t in tqdm(self.timepoints):
            ...
            for label in unique_labels:
                ...
                spline_resolution = int(len(label_coords) * self.best_intensity_resolution_factor)
                ...
                # Interpolate curvature to match the length of skeleton_path
                curvature_intensity = np.interp(
                    np.linspace(0, 1, len(skeleton_path)),  # Interpolating points
                    np.linspace(0, 1, len(curvature_intensity)),  # Original curvature points
                    curvature_intensity  # Curvature values
                )
                ...
                spline_resolution = int(len(label_coords) * self.best_structural_resolution_factor)
                ...
                # Interpolate curvature to match the length of skeleton_path
                curvature_structural = np.interp(
                    np.linspace(0, 1, len(skeleton_path)),  # Interpolating points
                    np.linspace(0, 1, len(curvature_structural)),  # Original curvature points
                    curvature_structural  # Curvature values
                )
        ...
        self.plot_mean_curvature_over_time(weighted_mean_curvatures_structural,
                                           ...
                                           save_folder=self.save_path, view_plot=False)

        # max project the path_im
        # path_im = np.max(path_im, axis=1)
        if self.show_summary_plots:
            self.napari_view([intensity_im, skeleton_im, path_im, structural_curvature_im, intensity_curvature_im],
                             IMnames=['Intensity', 'Skeleton', 'Paths', 'Structural Curvature', 'Intensity Curvature'],
