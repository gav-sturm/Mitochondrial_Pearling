...
def calculate_spontaneous_curvature(images, channels=None, microns_per_pixel=0.116, visualize=False):
    ...
    mean_curvatures = []
    sem_values = []
    skeletons = np.zeros_like(images)
    binary_images = np.zeros_like(images)
    spline_images = np.zeros_like(images)
    ...
    if len(images.shape) == 4:  # (T, C, X, Y)
        ...
        for channel in channels:
            ...
            for t in range(T):
                ...
                mean_curvature, sem, binary_images[t,channel,:,:], skeletons[t,channel,:,:], curvature_image[t,channel,:,:] = process_image(image, microns_per_pixel, t=t)
        ...
        if images.shape[0] > 1:  # (T, X, Y) - multiple time points, no channels
            ...
            for t in range(T):
                image = images[t, :, :]
                mean_curvature, sem, binary_images[t,:,:], skeletons[t,:,:], curvature_image[t,:,:] = process_image(image, microns_per_pixel, t=t)
                mean_curvatures.append(mean_curvature)
                sem_values.append(sem)
            ...
            for channel in channels:
                ...
                mean_curvature, sem, binary_images[:,:], skeletons[channel,:,:], curvature_image[channel,:,:] = process_image(image, microns_per_pixel)
                mean_curvatures.append(mean_curvature)
                sem_values.append(sem)
    elif len(images.shape) == 2:  # (X, Y) - single image, no channels, no time points
        mean_curvature, sem, binary_images, skeletons, curvature_image = process_image(images, microns_per_pixel)
        mean_curvatures.append(mean_curvature)
        sem_values.append(sem)
    ...
    if visualize:
        ...
        viewer.add_image(skeletons, name='Contours')
...
def spontaneous_curvature(spline_points, window_size=10, t=0):
    ...
    # Normalize curvature values for heatmap display
    curvature_normalized = (curvatures - np.min(curvatures)) / (np.max(curvatures) - np.min(curvatures))

    # Plotting the curvature heatmap
    if t == 37:
        plt.scatter(x_coords, y_coords, c=curvature_normalized, cmap='viridis', s=50, edgecolor='k', linewidths=0.1, alpha=0.8)
        plt.xlabel('X Coordinates')
        plt.ylabel('Y Coordinates')
        plt.title('Spline Points with Curvature Heatmap')
        plt.colorbar(label='Spontaneous Curvature')
    ...
    spontaneous_curvature_value = total_curvature / length if length != 0 else 0

    return spontaneous_curvature_value, sem, length
...
def process_image(image, microns_per_pixel=0.116, sigma=5, max_curvature=100, t=0):
    ...
    for contour in contours:
        ...
        if len(contour) >= 4:  # Ensure there are enough points to calculate curvature
            ...
            curvature, sem, length = spontaneous_curvature(spline_points, window_size=50, t=t)
    ...
    return normalized_spontaneous_curvature, normalized_sem, binary_image, contour_image, curvature_image
...
if __name__ == "__main__":
    file_path = r"F:\UCSF\W1_FRAP\2024-01-18_Laurdan_v2\ctrl\hFB13_PKMO_laurdan_ctrl_100lp_2-insane-pearling\hFB13_PKMO_laurdan_ctrl_2-ij.tif"
